...
def main(self):
    ...
    Summary:
        - Orchestrates the entire analysis pipeline:
            - Fetches data from Google Sheets for each sheet.
            - Processes data independently for "Navis" and "Two Fat Cats".
            - Creates unique customer IDs.
            - Calculates visit frequencies and recency.
            - Categorizes customers into meaningful groups for each sheet.
            - Prints the final customer categories for review.
    """

    try:
        # Fetch and process data separately for each sheet
        sheet_data = {}
        customer_categories = {}

        for worksheet_name in self.worksheet_names:
            # Fetch data for the specific sheet
            data = self.get_data_from_google_sheet(self.sheet_id, worksheet_name, self.credentials_file)

            # Create Customer IDs
            data = self.create_customer_id(data)

            # Calculate monthly visits and recency
            monthly_visits = self.calculate_monthly_visits(data)
            recency_data = self.calculate_recency(data)

            # Categorize customers
            categories = self.categorize_customers(monthly_visits, recency_data)

            # Store processed data and results for each sheet
            sheet_data[worksheet_name] = data
            customer_categories[worksheet_name] = categories

        # Print the results for each sheet
        for worksheet_name, categories in customer_categories.items():
            print(f"Customer Categories for {worksheet_name}:", categories)
